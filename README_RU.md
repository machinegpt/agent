...
graph TD
    subgraph Engine["AISynthesisEngine (jinx_test.py)"]
        ...
        DIFF["2. Детектор дрейфа API"]
        GEN["3. Синтезатор динамических тестов"]
        AST --> DIFF --> GEN
    end
...
Оркестратор тестов выполняет 9 независимых диагностических фаз, сгруппированных в 5 ключевых направлений:
