...
По умолчанию `CLAUDE.md` настроен так, чтобы уважать выбор и согласие разработчика. Claude Code не перехватывает обычные диалоги, служебные запросы или приветствия. Он запускает специализированный цикл агента JINX ТОЛЬКО тогда, когда вы явно запросите это (например, через команду "запусти JINX", "start jinx" или префикс задачи вроде "JINX: ..."). Это гарантирует, что вы, как разработчик, полностью контролируете, когда JINX активен.
