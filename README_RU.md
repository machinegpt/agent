...
Для сквозного перехвата управления в корневом каталоге проекта размещен декларативный манифест `CLAUDE.md`, предписывающий хост-среде Claude Code безусловно перенаправлять любые входящие пользовательские транзакции (включая служебные сообщения и базовые приветствия) на обработку ядру JINX.

### Декларативная Авторизация Прав Исполнения (Non-Interactive Sandbox Mode)

По умолчанию политика безопасности Claude Code CLI требует интерактивного подтверждения оператора для каждой операции изменения файловой структуры и выполнения команд интерпретатора Python. Для обеспечения автономного и неблокирующего функционирования когнитивного цикла JINX (полная автоматизация без ручного подтверждения) необходимо сконфигурировать глобальный профиль безопасности хост-редактора.
...
#### Архитектурное назначение параметров авторизации:
...
1. Инициализируйте сессию консольного интерфейса Claude Code в корневом каталоге проекта:
   ...
   claude
   ```
2. Отправьте бизнес-задачу или интерактивный запрос непосредственно в поток сессии в свободной лингвистической форме:
   * *«Добавь функцию деления в calc.py и проверь тестами»*
   * *«Привет, опиши текущий статус проекта»*

В соответствии с правилами `CLAUDE.md`, хост выполнит инициализацию дочернего оркестратора `python .agent/jinx.py "[запрос]"` и прозрачно проведет сессию через все транзакционные раунды когнитивного цикла до полной верификации поставленной задачи.
