...
默认情况下，`CLAUDE.md` 的配置尊重用户的控制权与知情同意。Claude Code 不会拦截您的日常对话、管理查询或简单问候。它**仅**在您明确请求时（例如使用 "run JINX"、"start jinx" 或带有 "JINX: ..." 等任务前缀）才会启动 JINX 的专业代理循环。这确保了您（开发人员）能够完全掌控 JINX 的启动时机。
